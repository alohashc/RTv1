...
void	ft_light_sph(t_main *main, t_vec dir)
{
	...
	while (i < main->num)
	{
		...
		main->ray[i].n = ft_diff(main->ray[i].p, main->sph[main->index].center_sph);
...
double	ft_disc_sph(t_main *main, t_vec dir, t_vec pos, int i)
{
	...
	dis.c = ft_dot_prod(main->sph[i].inter.diff, main->sph[i].inter.diff) - radius * radius;
...
void	ft_tracing_sphere(t_main *main)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			...
			if(ft_intersect_sph(main, dir, main->cam.c_position, 0) != 0)
			{
				ft_light_sph(main, dir);
				mlx_pixel_put(main->win.mlx, main->win.win, x, y, ft_color(main, main->sph[0].col));
			}
		}
	}
}
